 ...
 *
 * @module dsh-plugin-cron-scheduler/routes
 */
...
/**
 * List session choices as { id, label } for the fixed-session dropdown.
 *
 * Two sources, merged and de-duplicated:
 *  1. live sessions (sessions.list()) — titled when sessionTitle has one;
 *  2. every fixedSessionId already bound by a job — so a previously-bound
 *     session stays selectable even after a restart, when it is persisted
 *     but no longer live (the live-only store list would silently drop it).
 */
export function listSessionChoices(ctx, store) {
  ...
  // Persisted-but-not-live sessions referenced by existing jobs.
  for (const job of store.list()) {
    if (job.sessionStrategy !== 'fixed' || !job.fixedSessionId) continue
    const id = String(job.fixedSessionId)
    if (seen.has(id)) continue
    out.push({ id, label: `${id.slice(0, 8)}…（已绑定）` })
    seen.add(id)
  }
...
export function registerRoutes(ctx, store, deps = {}) {
  ...
  const listSessionChoicesBound = () => listSessionChoices(ctx, store)
