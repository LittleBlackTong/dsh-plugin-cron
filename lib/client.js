 ...
 *
 * 自适应（v0.2.9，方案 A+B）：
 *  侧栏宽度可被用户拖动改小，footer 槽位又是 row 方向 flex，因此本面板：
 *  - A：不设硬宽度（width:100% / min-width:0 / box-sizing:border-box），行内元素
 *       允许省略号收缩（Cron 表达式 flex-shrink:2），按钮与状态 flex-shrink:0，
 *       列表容器 overflow-x 强制 hidden（否则 overflow-y:auto 会连带打开横向滚动条）；
 *  - B：按 ResizeObserver 实测宽度分档（full/compact/minimal）决定 Cron 表达式与
 *       状态文字的显隐，被省略的信息挂 title tooltip。任何宽度下按钮都不会被挤出。
 *
 * 注：v0.2.8 曾把列表搬到 shell.overlay 弹层（footer 只留入口），v0.2.9 按反馈回退为
 * 「任务列表直接内联在会话窗口里」，只保留上面的自适应能力。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- 主面板（内联在侧栏 footer：任务列表直接可见，不开弹层） ----
    function CronPanel({ wide }) {
      ...
      // 宽度分档：footer 是 row 方向 flex，宿主宽度会被用户拖动改小，
      ...
      return React.createElement(SectionBoundary, null,
        React.createElement('div', { ref: panelRef, style: { padding: '6px 8px 14px', width: '100%', minWidth: 0, boxSizing: 'border-box' } },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px', minWidth: 0, marginBottom: '4px' } },
            React.createElement('span', { style: { fontSize: '12px', fontWeight: 600, color: V.text, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '定时任务'),
