...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- 主题 token（全部来自 Theme.listTokens；不设深色 fallback） ----
    const V = {
    ...
    function Switch({ on, onClick, disabled }) {
      return React.createElement('button', {
        ...
        style: {
          ...
          background: on ? V.primary : 'var(--dsw-alias-label-secondary)',
    ...
    // ---- 任务行 ----
    function JobRow({ job, busy, onToggle, onEdit, onDelete, onRun }) {
      const st = statusText(job)
      return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 0', borderBottom: '1px solid ' + V.border } },
        ...
        React.createElement('div', { style: { flex: 1, minWidth: 0, overflow: 'hidden' } },
          React.createElement('div', { style: { fontSize: '12px', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: V.text } }, job.name),
          React.createElement('div', { style: { fontSize: '10px', color: V.text2 } },
            job.schedule + ' · ' + formatTime(job.nextRunAt),
          ),
          React.createElement('div', { style: { fontSize: '10px', color: st.color } },
            '上次: ' + st.text + ' ' + formatTime(job.lastRunAt),
            (job.lastRunStatus === 'failed' && job.lastRunError) ? ' (' + String(job.lastRunError).slice(0, 40) + ')' : '',
          ),
        ),
        React.createElement('button', { style: btnGhost(false), onClick: () => onRun(job), title: '立即运行', disabled: busy }, '跑'),
    ...
    function Dialog({ title, onClose, footer, children }) {
      return React.createElement('div', {
        ...
        React.createElement('div', { style: { background: V.overlay, border: '1px solid ' + V.border, borderRadius: '10px', width: '420px', maxWidth: '92vw', maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 12px 40px rgba(0,0,0,.25)' } },
        ...
        React.createElement('div', { style: { padding: '6px 8px' } },
          ...
          React.createElement('div', { ref: listRef, style: { maxHeight: maxH, overflowY: 'auto' } },
            (jobs || []).map((job) => React.createElement(JobRow, {
              ...
              onDelete: (j) => setModal({ kind: 'delete', job: j }), onRun: runJob,
            })),
          ),
          modal !== undefined && modal.kind === 'form' ? React.createElement(JobFormModal, {
            job: modal.job, onClose: () => setModal(undefined), onSaved: refresh,
          }) : null,
