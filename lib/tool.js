 ...
 *
 * @module dsh-plugin-cron/tool
 */

import { parseCron, nextMatch } from './cron.js'

export function registerTool(ctx, store) {
  return ctx.tools.register({
    name: 'cron_manage',
    description: '管理定时任务：创建(create)、列出(list)、更新(update)、删除(delete)、切换开关(toggle)。自然语言由你在对话中解析，本工具只接收结构化参数。',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['create', 'list', 'update', 'delete', 'toggle'], description: '操作类型' },
        id: { type: 'string', description: '任务ID（update/delete/toggle必填）' },
    ...
    async execute(args) {
      ...
      switch (action) {
        ...
        case 'update': {
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, job, summary: `已更新「${job.name}」` }) }],
          }
        }
        case 'delete': {
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: ok, deleted: ok }) }],
          }
        }
        case 'toggle': {
          ...
          return {
            content: [{ type: 'text', text: JSON.stringify({ success: true, job, summary: `已${args.enabled ? '启用' : '暂停'}「${job.name}」` }) }],
          }
        }
        default:
