 ...
 * @param {{ cwd?: string, selection?: object, waiter?: object }} options
...
async function resolveAgentAndDeliver(ctx, sessionId, message, options = {}) {
  const { selection, waiter } = options
  ...
  if (live) {
    ...
    // Pin the delivery position before followup(): any turn/end the wake-up
    // appends — even synchronously — must land at seq >= firstSeq.
    waiter?.settle(firstSeq)
 ...
 *
 * The outcome arrives through the waiter armed by `armTurnWaiter()` (see
 * CronScheduler): the scheduler observes the session's live `turn/end` via the
 * public `session/event` Cordis event. Session has no event-log reader for new
 * code (`snapshotEvents()` is deprecated), so a missing waiter — only possible
 * where `ctx.on` itself is absent — is treated as unobservable rather than hung.
 * @param {{ promise: Promise<{ reason?: object, timedOut?: boolean }> }} [waiter]
 * @returns {Promise<{ error?: string }>}
 */
async function settleOutcome(waiter) {
  if (waiter === undefined) return {}
  const { reason, timedOut } = await waiter.promise
...
export class CronScheduler {
  ...
  constructor({ store, ctx, cwd }) {
    ...
    /** sessionId → waiters waiting for that session's next `turn/end`. */
    this.pendingTurns = new Map()
    // One live subscription shared by all fires. Session exposes no event-log
    // reader for new code (`snapshotEvents()` is deprecated), so `session/event`
    // — the public Cordis seam in-repo observers use — is how a fired turn's
    // `turn/end` is seen. Registered here rather than in start() because fire()
    // is also invoked directly (manual trigger tests/headless use).
    this.disposeSessionListener = typeof ctx?.on === 'function'
      ? ctx.on('session/event', (session, event) => this.onSessionEvent(session, event))
      : undefined
  }

  /**
   * Match an appended `turn/end` to the session's waiters. A waiter only
   * accepts events at seq >= the delivery position pinned by `settle()`, so
   * endings of turns that predate the cron delivery are never mistaken for
   * the fired turn's outcome.
   */
  onSessionEvent(session, event) {
    if (event?.type !== 'turn/end') return
    const waiters = this.pendingTurns.get(String(session?.id))
    if (waiters === undefined || waiters.length === 0) return
    const reason = event.data?.reason
    for (const waiter of [...waiters]) {
      if (event.seq < waiter.firstSeq) continue
      waiter.finish({ reason })
    }
  }

  /** Remove a pending waiter; false when it already finished. */
  takeTurnWaiter(sessionId, waiter) {
    const waiters = this.pendingTurns.get(sessionId)
    const index = waiters?.indexOf(waiter) ?? -1
    if (index === -1) return false
    waiters.splice(index, 1)
    if (waiters.length === 0) this.pendingTurns.delete(sessionId)
    return true
  }

  /**
   * Arm a turn-end waiter for `sessionId` BEFORE the message is delivered and
   * pin it to the delivery position (`settle`) right before `followup()`. The
   * retired log-polling could re-read history after the fact; live events
   * cannot, so a turn that fails within milliseconds of delivery would
   * otherwise be missed and the run would hang until the timeout.
   * @param {string} sessionId
   * @returns {{ promise: Promise<{ reason?: object, timedOut?: boolean }>, settle: (firstSeq: number) => void, disarm: () => void } | undefined}
   *   undefined when no live event seam exists (unobservable, not hung).
   */
  armTurnWaiter(sessionId) {
    if (this.disposeSessionListener === undefined) return undefined
    const waiter = { firstSeq: Infinity, finish: undefined, timer: undefined }
    waiter.promise = new Promise((resolve) => {
      waiter.finish = (result) => {
        if (!this.takeTurnWaiter(sessionId, waiter)) return
        clearTimeout(waiter.timer)
        resolve(result)
      }
    })
    waiter.timer = setTimeout(() => waiter.finish({ timedOut: true }), TURN_TIMEOUT_MS)
    waiter.timer.unref?.()
    let waiters = this.pendingTurns.get(sessionId)
    if (waiters === undefined) {
      waiters = []
      this.pendingTurns.set(sessionId, waiters)
    }
    waiters.push(waiter)
    return {
      promise: waiter.promise,
      settle: (firstSeq) => { waiter.firstSeq = firstSeq },
      disarm: () => { if (this.takeTurnWaiter(sessionId, waiter)) clearTimeout(waiter.timer) },
    }
  }
  ...
  stop() {
    ...
    this.timers.clear()
    // Pending turn waiters are left to their own timeout, mirroring the
    // retired polling flow (whose loops also outlived stop()); with the
    // listener gone a turn ending after stop() settles as timed-out.
  }
  ...
  async fire(job) {
    ...
    try {
      ...
      const message = {
        ...
        // Producer-owned source kind: the v4 session format refuses the retired
        // `kind: 'plugin'` wrapper at persistence admission, and `plugin:cron`
        // is exactly what the harness's v3→v4 migration mints for this plugin.
        source: { kind: 'plugin:cron', jobId: current.id },
      }
      // Arm before delivery: a turn that fails within milliseconds of
      // followup() (first-turn prompt-assembly errors) must still be seen.
      turnWaiter = this.armTurnWaiter(sessionId)
      ...
      if (!delivered.ok) {
        ...
        const outcome = await settleOutcome(turnWaiter)
      ...
      // Release the waiter when delivery failed or fire threw; a no-op once
      // the waiter finished (success/failure/timeout all consume it first).
      turnWaiter?.disarm()
