 ...
 *
 * @module dsh-plugin-cron-scheduler/store
 */
 ...
 *
 * The harness serializes tool results as lossless JSON, which rejects
 * `undefined` outright (`value is not lossless JSON`). A job has several
 * optional fields (`fixedSessionId`, `lastRunAt`, `lastRunStatus`,
 * `lastRunError`), so every copy that leaves the store must omit those keys
 * when unset instead of carrying them with an `undefined` value — otherwise
 * `cron_manage` reports a spurious failure even though the write succeeded.
