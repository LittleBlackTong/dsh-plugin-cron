...
- **UI 位置（v0.2.8 起）**：侧边栏底部 `sidebar.footer.action` 只放一个**紧凑入口**（时钟 + 「定时任务」+ 任务数 + 失败红点；侧栏收起时只显示时钟）。点开后在 **`shell.overlay` 弹层**里管理任务（新建/编辑/删除/启停/立即运行），`Esc` 或点遮罩关闭 —— 与 Cordis Plugin 面板并列，**不覆盖、不替换**它。
- **自适应**：入口本身宽度无关（任何侧栏宽度都不溢出）；弹层里的任务行按容器实测宽度自动分档（`ResizeObserver`）：
  ...
  被省略的名称/表达式/状态都带 `title`，悬停可看全文。任务多时列表在弹层内滚动（上限 420px 高）。
